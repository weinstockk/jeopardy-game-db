...
export default function SetupScreen({ setLoading, navigate, setAppState }: Props) {
  const [view, setView] = useState<View>('home')
  const [name, setName] = useState('')
  const [diff, setDiff] = useState<'easy' | 'medium' | 'hard'>('easy')
  const [error, setError] = useState('')
  const [activeGames, setActiveGames] = useState<{ game_id: string; player_name: string; round: number; remaining: number }[]>([])
  const [history, setHistory] = useState<{ game_id: string; player: { name: string; score: number }; bots: { name: string; score: number }[]; winner: { name: string; score: number }; completed_at: number }[]>([])
            ...
            {activeGames.length === 0
              ...
              : activeGames.map(g => (
                <div key={g.game_id} className="game-item">
                  ...
                  <button className="btn-primary btn-sm" onClick={() => resumeGame(g.game_id)}>Resume</button>
                </div>
              ))
            }
            <button className="btn-ghost" style={{ marginTop: '1rem' }} onClick={() => setView('home')}>← Back</button>
          </div>
        )}
              ...
              : history.map(g => {
                ...
                return (
                  <div key={g.game_id} className="game-item" style={{ display: 'block' }}>
                    ...
                    {date && <div className="gi-sub" style={{ color: '#555', marginTop: '0.1rem' }}>{date}</div>}
                  </div>
                )
              })
            }
