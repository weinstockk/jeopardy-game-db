            ...
            <div className="results-list">
              ...
              {result.all_buzz_times.map(p => (
                <div key={p.name} className="result-row">
                  <span className="rr-name">{p.is_player ? playerName : p.name}{p.is_player ? ' (you)' : ''}</span>
                  <span className="rr-ans">{p.buzz_time >= 999 ? '—' : `${p.buzz_time.toFixed(2)}s`}</span>
                </div>
              ))}
                ...
                <>
                  ...
                  {result.results.map((r, i) => (
                    <div key={i} className="result-row">
                      ...
                      <span className={r.correct ? 'rr-ok' : 'rr-bad'}>{r.correct ? '✓' : '✗'}</span>
                    </div>
                  ))}
                </>
              )}
              {result.round_winner ? (
                <div style={{ marginTop: '0.5rem', textAlign: 'center', color: '#aaa', fontSize: '0.78rem' }}>
...
function AnswerGrid({
  ...
  answers: string[]; selected: string | null; correct: string | null; onSelect: (a: string) => void; disabled?: boolean
}) {
  return (
