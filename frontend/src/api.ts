...
export const api = {
  newGame: (player_name: string, ai_difficulty: string) =>
    req<{ game_id: string; player: string; ai_1: string; ai_2: string }>('/api/new-game', 'POST', { player_name, ai_difficulty }),

  getGame: (gameId: string) =>
    req<import('../src/App').GameState>(`/api/game/${gameId}`),

  activeGames: () =>
    req<{ game_id: string; player_name: string; round: number; remaining: number; created_at: string }[]>('/api/active-games'),
  ...
  submitAnswer: (gameId: string, category: string, value: number, answer: string, player_buzz_time: number, bot_buzz_times: Record<string, number>) =>
    req<{
      ...
      results: { name: string; display_name: string; is_player: boolean; answer: string; correct: boolean; buzz_time: number }[];
  ...
  dailyDouble: (gameId: string, category: string, value: number, wager: number, answer: string) =>
    req<{ player_correct: boolean; correct_answer: string; wager: number; delta: number; remaining: number; game_over: boolean }>(
  ...
  submitFinal: (gameId: string, wager: number, answer: string) =>
    req<{
      ...
      reveal: { name: string; is_player: boolean; correct: boolean; wager: number; delta: number; answer: string }[]
    }>(`/api/final/${gameId}`, 'POST', { wager, answer }),

  endGame: (gameId: string) =>
    req<{ winner: { name: string; score: number }; all_scores: { name: string; score: number }[] }>(`/api/end-game/${gameId}`, 'POST'),
}